...
public class BlockLeavesOrange extends BlockLeavesBase
{
    ...
    protected BlockLeavesOrange(int par1, int par2)
    {
            ...
            for (int k = -i; k <= i; k++)
            {
                for (int l = -i; l <= i; l++)
                {
                    for (int i1 = -i; i1 <= i; i1++)
                    {
                        ...
                        if (j1 == mod_LeavesOrange.leavesOrange.blockID)
                        {
                ...
                for (int i1 = -byte0; i1 <= byte0; i1++)
                {
                    for (int l1 = -byte0; l1 <= byte0; l1++)
                    {
                        for (int j2 = -byte0; j2 <= byte0; j2++)
                        {
                            ...
                            if (l2 == mod_LeavesOrange.leavesOrange.blockID)
                            {
    ...
    public void harvestBlock(World par1World, EntityPlayer par2EntityPlayer, int par3, int par4, int par5, int par6)
    {
        if (!par1World.isRemote && par2EntityPlayer.getCurrentEquippedItem() != null && par2EntityPlayer.getCurrentEquippedItem().itemID == Item.shears.shiftedIndex)
        {
            ...
            dropBlockAsItem_do(par1World, par3, par4, par5, new ItemStack(mod_LeavesOrange.leavesOrange.blockID, 1, par6 & 3));
        }
        else
        {
