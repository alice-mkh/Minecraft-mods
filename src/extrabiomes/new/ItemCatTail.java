...
public class ItemCatTail extends Item
{
    ...
    public ItemCatTail(int i, Block block)
    {
        super(i);
        spawnID = mod_CatTail.catTail.blockID;
    }
    ...
    public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int i, int j, int k, int l)
    {
        ...
        if (world.canBlockBePlacedAt(spawnID, i, j, k, true, l))
        {
            ...
            if (world.setBlockWithNotify(i, j, k, spawnID))
            {
                ...
                world.playSoundEffect((float)i + 0.5F, (float)j + 0.5F, (float)k + 0.5F, block.stepSound.getStepSound(), (block.stepSound.getVolume() + 0.5F) / 2.0F, block.stepSound.getPitch() * 1.3F);
                itemstack.stackSize--;
            }
        }
        return true;
